...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const diffInMs = new Date(dateStart) - new Date(dateEnd);
  return Math.abs(diffInMs / (1000 * 60 * 60 * 24)) + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const weekends = days.filter((day) => day === 6 || day === 0);
...
function getWeekNumberByDate(date) {
  ...
  const firstDay = new Date(y, 0, 2);
  while (firstDay.getDay() !== 1) {
    firstDay.setDate(firstDay.getDate() - 1);
  }
  let res = Math.abs(Math.ceil((date - firstDay) / (1000 * 60 * 60 * 24) / 7));
  if (
    ((date - firstDay) / (1000 * 60 * 60 * 24)) % 7 === 0 ||
    date.getDay() === 0
  )
    res += 1;
  return res;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  function changeDDwithMM(dateStr) {
    let date = '';
  ...
  while (start.getTime() - end.getTime() !== 0) {
    while (work > 0) {
      if (start.getTime() - end.getTime() === 0) break;
      schedule.push(
        `${start.getDate() > 9 ? start.getDate() : `0${start.getDate()}`}-${start.getMonth() + 1 > 9 ? start.getMonth() + 1 : `0${start.getMonth() + 1}`}-${start.getFullYear()}`
      );
